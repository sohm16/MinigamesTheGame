...
public class PlatformRaceController : MonoBehaviour {
	...
	void Start () {
		...
		cam.GetComponent<Rigidbody> ().velocity = new Vector3 (0.75f, 0, 0); // set camera movement speed
	...
	void Update () {
		...
		if (!onlyOnce && gc.gameTime <= 0) {	// make sure you only subtract one life from the loser (false by default)
			...
			cam.GetComponent<Rigidbody>().velocity = new Vector3 (-0.5f,0,0);

			if (p1.transform.position.x > p2.transform.position.x + 0.25f) { // if someones further than the other, they win
				gc.gameOver (1);
				p1.GetComponent<Animator>().SetTrigger ("Win");
				p2.GetComponent<Animator>().SetTrigger ("Lose");
			}
				
			else if (p1.transform.position.x + 0.25f < p2.transform.position.x) {
				gc.gameOver (2);
